...
namespace NO_SRS.Data
{
    ...
    public class SRSRadioReader
    {
        ...
        public bool findInstance()
        {
            try
            {
                ...
                foreach (var obj in heap.EnumerateObjects())
                {
                    ...
                    instanceAddr = obj.Address;
                    log.LogInfo($"[SRS] ClientStateSingleton found at 0x{instanceAddr:X}");
                    initialised = true;
                    return true;
                }
        ...
        public SRSRadio readState()
        {
            ...
            try
            {
                ulong baseAddr = instanceAddr + OBJ_HEADER; //Singleton address
                ...
                {
                    log.LogError($"[NO SRS] Could not read radio array pointer");
                    return null;
                }
                ...
                ulong sendingStatePtr = readPtr(instanceAddr + OBJ_HEADER + RADIO_SENDING_STATE_OFFSET);
                ...
                ulong reciveingArrayPtr = readPtr(instanceAddr + OBJ_HEADER + RADIO_RECEIVING_STATE_OFFSET);
                long lastReceived = 0;
